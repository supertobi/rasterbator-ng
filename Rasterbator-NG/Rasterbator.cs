...
namespace Rasterbator
{
    ...
    {
        readonly NameValueCollection Attr = null;
        ...
        /// <summary>
        /// Determines whether Rasterbator-NG has timed out.
        /// </summary>
        /// <remarks>
        ...
        public void CreateImage()
        {
            ...
            Document = new Document(new iTextSharp.text.Rectangle(psx, psy), 0, 0, 0, 0);
            PdfWriter = PdfWriter.getInstance(Document, new FileStream(Attr["TargetFilename"].ToString(), FileMode.Create));
            PdfWriter.setViewerPreferences(PdfWriter.FitWindow);
            Document.addTitle("Rasterbation");
            Document.addCreator("The Rasterbator at https://github.com/supertobi/rasterbator-ng");
            Document.Open();
        ...
        private void RasterbatePage(int px, int py)
        {
            ...
            if (!UseAverageColor) PdfContentByte.setRGBColorFill(R, G, B);
            ...
            for (int sy = -1; sy < SquaresY + 1; sy++)
            {
                ...
                for (int sx = -1; sx < SquaresX + 1; sx++)
                {
                    ...
                    if (UseAverageColor) PdfContentByte.setRGBColorFill((int)(r / coc), (int)(g / coc), (int)(b / coc));

                    if (Radius / 2 > 0) PdfContentByte.circle(ex, psy - ey, Radius / 2);
            ...
            PdfContentByte.rectangle(-SquareSize, 0, psx + SquareSize, -SquareSize); // top
            PdfContentByte.fill();
            PdfContentByte.rectangle(-SquareSize, -SquareSize, 0, SquareSize); // left
            PdfContentByte.fill();
            PdfContentByte.rectangle(rx, -SquareSize, SquareSize, psy + SquareSize); // right
            PdfContentByte.fill();
            PdfContentByte.rectangle(-SquareSize, psy - ry, psx + SquareSize, -SquareSize); // bottom
            PdfContentByte.fill();

            if (CroppingRectangle)
            {
                PdfContentByte.LineWidth = 0.5f;
                PdfContentByte.moveTo(0, psy);

                PdfContentByte.setRGBColorStroke(0xbb, 0xbb, 0xbb);

                PdfContentByte.lineTo(rx, psy);
                PdfContentByte.lineTo(rx, psy - ry);
                PdfContentByte.lineTo(0, psy - ry);
                PdfContentByte.lineTo(0, psy);
